...
import { Phone, ArrowRight, AlertTriangle, Home } from "lucide-react"
...
export async function generateMetadata({
  ...
  return {
    title: dict.accuServiceAanHuis?.seo?.title || "Accu service aan huis | 24/7 Accu Hulp & Startservice",
    description: dict.accuServiceAanHuis?.seo?.description || "Accu service aan huis nodig? 24/7 bereikbaar voor snelle hulp, vervanging en startservice bij u thuis. Wij helpen u direct.",
    keywords: dict.accuServiceAanHuis?.seo?.keywords || "accu service aan huis, accu vervangen thuis, startservice, accu hulp, auto accu leeg",
    openGraph: {
      title: dict.accuServiceAanHuis?.seo?.title || "Accu service aan huis | 24/7 Accu Hulp & Startservice",
      description: dict.accuServiceAanHuis?.seo?.description || "Accu service aan huis nodig? 24/7 bereikbaar voor snelle hulp, vervanging en startservice bij u thuis. Wij helpen u direct.",
      type: "website",
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: dict.accuServiceAanHuis?.seo?.imageAlt || "Accu service aan huis",
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `/${locale}/accu-service-aan-huis`,
    },
  }
}

export default async function AccuServiceAanHuisPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
              ...
              <div className="space-y-8">
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-2 bg-[#c8eb67] text-black px-4 py-2 rounded-full text-sm font-medium">
                    ...
                    {dict.accuServiceAanHuis?.hero?.badge || "Accu service aan huis"}
                  </div>
                  <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold leading-tight">
                    <span className="text-white">{dict.accuServiceAanHuis?.hero?.title?.part1 || "Accu service aan huis nodig?"}</span>
                    <br />
                    <span className="text-[#c8eb67]">{dict.accuServiceAanHuis?.hero?.title?.part2 || "Wij komen direct naar u toe"}</span>
                  </h1>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <a
                    href={`tel:${dict.accuServiceAanHuis?.hero?.phone || "0852502928"}`}
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      {dict.accuServiceAanHuis?.hero?.cta || "Bel Direct: 085-250 29 28"}
                    </span>
                  </a>
                  <Link
                  ...
                  >
                    <span className="mr-3">{dict.accuServiceAanHuis?.hero?.whatsapp || "WhatsApp"}</span>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex justify-center">
              <Image
                src="/services/3.jpg"
                alt={dict.accuServiceAanHuis?.tips?.imageAlt || "Accu service aan huis hulp"}
                width={500}
                height={333}
            ...
            <div>
              <div className="text-center md:text-left mb-10">
                <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                  {dict.accuServiceAanHuis?.tips?.title || "Wat houdt accu service aan huis in?"}
                </h2>
                <p className="text-base sm:text-xl text-gray-600 max-w-3xl">
                  {dict.accuServiceAanHuis?.tips?.subtitle ||
                    "Wij komen bij u thuis voor accuproblemen, vervangen uw accu ter plaatse en zorgen dat u snel weer kunt rijden."}
                </p>
              </div>
              <div className="space-y-8">
                {(dict.accuServiceAanHuis?.tips?.items || [
                  { title: "Accu vervangen aan huis", description: "Wij vervangen uw accu direct bij u thuis." },
                  { title: "Startservice op locatie", description: "Wij starten uw auto veilig en snel bij u thuis." },
        ...
        <div className="container mx-auto px-6 flex flex-col-reverse md:flex-row items-center gap-12">
          <div className="md:w-3/5 text-center md:text-left">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              {dict.accuServiceAanHuis?.locatiesectie?.title || "Deel uw locatie voor hulp"}
            </h2>
            <p className="text-gray-700 mb-8 text-base sm:text-lg leading-relaxed">
              {dict.accuServiceAanHuis?.locatiesectie?.description ||
                "Deel uw locatie zodat wij u snel kunnen helpen en uw accu kunnen controleren of vervangen bij u thuis."}
            </p>
            <LocationButton
              dict={dict.accuServiceAanHuis?.locatiebutton || { sendLocation: "Deel Locatie", loading: "Laden..." }}
            />
          </div>
          <div className="md:w-2/5 flex justify-center mb-8 md:mb-0">
            <Image
              src="/location.gif"
              alt={dict.accuServiceAanHuis?.locatiesectie?.imageAlt || "Locatie delen voor hulp"}
              width={200}
              height={200}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mb-12">
            ...
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-6">
              {dict.accuServiceAanHuis?.emergency?.title || "Direct accu service aan huis nodig? Bel nu!"}
            </h2>
            <p className="text-lg sm:text-2xl text-black mb-12 max-w-3xl mx-auto">
              {dict.accuServiceAanHuis?.emergency?.subtitle ||
                "24/7 bereikbaar voor accuproblemen, startservice en vervanging aan huis. Snel ter plaatse, altijd veilig geholpen."}
            </p>
          </div>
          <div className="bg-black rounded-3xl p-8 md:p-12 mb-16 max-w-lg mx-auto">
            ...
            <a href={`tel:0852502928`}>
              <div className="text-3xl sm:text-5xl md:text-6xl text-white font-bold mb-4">
                {dict.accuServiceAanHuis?.emergency?.phone || "085-250 29 28"}
              </div>
            </a>
            <div className="text-white text-lg sm:text-xl">
              {dict.accuServiceAanHuis?.emergency?.phoneSubtitle || "Direct verbonden met onze hulpdienst"}
            </div>
          </div>
        </div>
      </section>
