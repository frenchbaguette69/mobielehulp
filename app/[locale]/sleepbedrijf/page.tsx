...
export async function generateMetadata({
  ...
  return {
    title: dict.sleepbedrijf?.seo?.title || "Sleepbedrijf - 24/7 Snel en Veilig Sleepdienst voor Auto's | Mobiele Hulp",
    description: dict.sleepbedrijf?.seo?.description || "Direct een sleepbedrijf nodig? 24/7 bereikbaar, snelle service, professioneel en veilig uw auto laten slepen of bergen.",
    keywords: dict.sleepbedrijf?.seo?.keywords || "sleepbedrijf, sleepdienst, auto slepen, auto berging, 24/7 sleepdienst, pechhulp, auto transport",
    openGraph: {
      title: dict.sleepbedrijf?.seo?.title || "Sleepbedrijf - 24/7 Snel en Veilig Sleepdienst voor Auto's",
      description: dict.sleepbedrijf?.seo?.description || "Direct een sleepbedrijf nodig? 24/7 bereikbaar, snelle service, professioneel en veilig uw auto laten slepen of bergen.",
      type: "website",
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: dict.sleepbedrijf?.seo?.imageAlt || "Professioneel sleepbedrijf",
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `/${locale}/sleepbedrijf`,
    },
  }
}

export default async function SleepbedrijfPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
              ...
              <div className="space-y-8">
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-2 bg-[#c8eb67] text-black px-4 py-2 rounded-full text-sm font-medium">
                    ...
                    {dict.sleepbedrijf?.hero?.badge || "Sleepbedrijf"}
                  </div>
                  <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold leading-tight">
                    <span className="text-white">{dict.sleepbedrijf?.hero?.title?.part1 || "Sleepbedrijf Nodig?"}</span>
                    <br />
                    <span className="text-[#c8eb67]">{dict.sleepbedrijf?.hero?.title?.part2 || "Snel & Veilig Gesleept"}</span>
                  </h1>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <a
                    href={`tel:${dict.sleepbedrijf?.hero?.phone || "0852502928"}`}
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      {dict.sleepbedrijf?.hero?.cta || "Bel Direct: 085-250 29 28"}
                    </span>
                  </a>
                  <Link
                  ...
                  >
                    <span className="mr-3">{dict.sleepbedrijf?.hero?.whatsapp || "WhatsApp"}</span>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              {dict.sleepbedrijf?.features?.title || "Waarom Ons Sleepbedrijf?"}
            </h2>
            <p className="text-base sm:text-xl text-gray-600 max-w-3xl mx-auto">
              {dict.sleepbedrijf?.features?.subtitle ||
                "Ons sleepbedrijf is 24/7 bereikbaar en zorgt voor veilig, snel en professioneel vervoer of berging van uw auto."}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {dict.sleepbedrijf?.features?.items?.map((feature, index) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-8">
                {dict.sleepbedrijf?.serviceDetails?.title || "Complete Sleepbedrijf Service"}
              </h2>
              <div className="space-y-6">
                {dict.sleepbedrijf?.serviceDetails?.items?.map((service, index) => (
            ...
            <div className="relative mt-10 lg:mt-0">
              <Image
                src="/services/8.jpg"
                alt={dict.sleepbedrijf?.serviceDetails?.imageAlt || "Professioneel sleepbedrijf"}
                width={600}
                height={400}
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              {dict.sleepbedrijf?.situations?.title || "Wanneer Sleepbedrijf Nodig?"}
            </h2>
            <p className="text-base sm:text-xl text-gray-600 max-w-3xl mx-auto">
              {dict.sleepbedrijf?.situations?.description ||
                "Of het nu gaat om pech, een ongeval of transport, ons sleepbedrijf helpt u snel en veilig."}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {dict.sleepbedrijf?.situations?.items?.map((situation, index) => (
              <div
                key={index}
        ...
        <div className="container mx-auto px-6 flex flex-col-reverse md:flex-row items-center gap-12">
          <div className="md:w-3/5 text-center md:text-left">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              {dict.sleepbedrijf?.locatiesectie?.title || "Deel Uw Locatie voor Sleepbedrijf"}
            </h2>
            <p className="text-gray-700 mb-8 text-base sm:text-lg leading-relaxed">
              {dict.sleepbedrijf?.locatiesectie?.description ||
                "Geef uw locatie door zodat wij u snel kunnen helpen en uw auto veilig kunnen vervoeren of bergen."}
            </p>
            <LocationButton
              dict={dict.sleepbedrijf?.locatiebutton || { sendLocation: "Deel Locatie", loading: "Laden..." }}
            />
          </div>
          <div className="md:w-2/5 flex justify-center mb-8 md:mb-0">
            <Image
              src="/location.gif"
              alt={dict.sleepbedrijf?.locatiesectie?.imageAlt || "Locatie delen voor sleepbedrijf"}
              width={200}
              height={200}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mb-12">
            ...
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-6">
              {dict.sleepbedrijf?.emergency?.title || "Direct Sleepbedrijf Nodig? Bel Nu!"}
            </h2>
            <p className="text-lg sm:text-2xl text-black mb-12 max-w-3xl mx-auto">
              {dict.sleepbedrijf?.emergency?.subtitle ||
                "24/7 bereikbaar voor sleepbedrijf. Snel ter plaatse, altijd veilig vervoer of berging."}
            </p>
          </div>
          <div className="bg-black rounded-3xl p-8 md:p-12 mb-16 max-w-lg mx-auto">
            ...
            <a href={`tel:0852502928`}>
              <div className="text-3xl sm:text-5xl md:text-6xl text-white font-bold mb-4">
                {dict.sleepbedrijf?.emergency?.phone || "085-250 29 28"}
              </div>
            </a>
            <div className="text-white text-lg sm:text-xl">
              {dict.sleepbedrijf?.emergency?.phoneSubtitle || "Direct verbonden met onze sleepbedrijf specialisten"}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {dict.sleepbedrijf?.emergency?.stats?.map((stat, index) => (
