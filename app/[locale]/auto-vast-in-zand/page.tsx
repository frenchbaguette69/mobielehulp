...
export async function generateMetadata({
  ...
  return {
    title: dict.autoVastInZand?.seo?.title || "Auto vast in zand? | 24/7 Sleepdienst & Hulp",
    description: dict.autoVastInZand?.seo?.description || "Is uw auto vast komen te zitten in het zand? 24/7 bereikbaar voor snelle hulp en sleepdienst. Wij helpen u snel en veilig.",
    keywords: dict.autoVastInZand?.seo?.keywords || "auto vast in zand, sleepdienst, auto bergen, pechhulp, auto uit zand halen",
    openGraph: {
      title: dict.autoVastInZand?.seo?.title || "Auto vast in zand? | 24/7 Sleepdienst & Hulp",
      description: dict.autoVastInZand?.seo?.description || "Is uw auto vast komen te zitten in het zand? 24/7 bereikbaar voor snelle hulp en sleepdienst. Wij helpen u snel en veilig.",
      type: "website",
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: dict.autoVastInZand?.seo?.imageAlt || "Auto vast in zand",
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `/${locale}/auto-vast-in-zand`,
    },
  }
}

export default async function AutoVastInZandPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
              ...
              <div className="space-y-8">
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-2 bg-[#c8eb67] text-black px-4 py-2 rounded-full text-sm font-medium">
                    ...
                    {dict.autoVastInZand?.hero?.badge || "Auto vast in zand"}
                  </div>
                  <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold leading-tight">
                    <span className="text-white">{dict.autoVastInZand?.hero?.title?.part1 || "Auto vast in zand?"}</span>
                    <br />
                    <span className="text-[#c8eb67]">{dict.autoVastInZand?.hero?.title?.part2 || "Wij helpen u direct"}</span>
                  </h1>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <a
                    href={`tel:${dict.autoVastInZand?.hero?.phone || "0852502928"}`}
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      {dict.autoVastInZand?.hero?.cta || "Bel Direct: 085-250 29 28"}
                    </span>
                  </a>
                  <Link
                  ...
                  >
                    <span className="mr-3">{dict.autoVastInZand?.hero?.whatsapp || "WhatsApp"}</span>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex justify-center">
              <Image
                src="/services/2.jpg"
                alt={dict.autoVastInZand?.tips?.imageAlt || "Auto vast in zand hulp"}
                width={500}
                height={333}
            ...
            <div>
              <div className="text-center md:text-left mb-10">
                <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                  {dict.autoVastInZand?.tips?.title || "Wat te doen als uw auto vast in het zand zit?"}
                </h2>
                <p className="text-base sm:text-xl text-gray-600 max-w-3xl">
                  {dict.autoVastInZand?.tips?.subtitle ||
                    "Volg deze stappen voor uw veiligheid. Lukt het niet? Wij helpen u direct!"}
                </p>
              </div>
              <div className="space-y-8">
                {(dict.autoVastInZand?.tips?.items || [
        ...
        <div className="container mx-auto px-6 flex flex-col-reverse md:flex-row items-center gap-12">
          <div className="md:w-3/5 text-center md:text-left">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              {dict.autoVastInZand?.locatiesectie?.title || "Deel uw locatie voor hulp"}
            </h2>
            <p className="text-gray-700 mb-8 text-base sm:text-lg leading-relaxed">
              {dict.autoVastInZand?.locatiesectie?.description ||
                "Deel uw locatie zodat wij u snel kunnen helpen en uw auto veilig kunnen bergen."}
            </p>
            <LocationButton
              dict={dict.autoVastInZand?.locatiebutton || { sendLocation: "Deel Locatie", loading: "Laden..." }}
            />
          </div>
          <div className="md:w-2/5 flex justify-center mb-8 md:mb-0">
            <Image
              src="/location.gif"
              alt={dict.autoVastInZand?.locatiesectie?.imageAlt || "Locatie delen voor hulp"}
              width={200}
              height={200}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mb-12">
            ...
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-6">
              {dict.autoVastInZand?.emergency?.title || "Direct hulp nodig? Bel nu!"}
            </h2>
            <p className="text-lg sm:text-2xl text-black mb-12 max-w-3xl mx-auto">
              {dict.autoVastInZand?.emergency?.subtitle ||
                "24/7 bereikbaar voor hulp bij auto's vast in het zand. Snel ter plaatse, altijd veilig geholpen."}
            </p>
          </div>
          <div className="bg-black rounded-3xl p-8 md:p-12 mb-16 max-w-lg mx-auto">
            ...
            <a href={`tel:0852502928`}>
              <div className="text-3xl sm:text-5xl md:text-6xl text-white font-bold mb-4">
                {dict.autoVastInZand?.emergency?.phone || "085-250 29 28"}
              </div>
            </a>
            <div className="text-white text-lg sm:text-xl">
              {dict.autoVastInZand?.emergency?.phoneSubtitle || "Direct verbonden met onze hulpdienst"}
            </div>
          </div>
        </div>
      </section>
