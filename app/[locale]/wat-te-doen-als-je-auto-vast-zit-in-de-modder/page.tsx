import { getDictionary } from "../dictionaries"
import { Phone, ArrowRight, AlertTriangle, Car } from "lucide-react"
import Link from "next/link"
import Image from "next/image"
import LocationButton from "@/components/locationbutton"
import { FAQSection } from "@/components/FAQsection"

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  const { locale } = await params
  const dict = await getDictionary(locale as "nl" | "en" | "fr" | "de")

  return {
    title: dict.modder?.seo?.title || "Auto vast in de modder? | Tips & Hulp",
    description: dict.modder?.seo?.description || "Is je auto vast in de modder? Lees wat je moet doen en krijg direct hulp. 24/7 bereikbaar voor pechhulp en berging.",
    keywords: dict.modder?.seo?.keywords || "auto vast, modder, auto losmaken, pechhulp, auto bergen",
    openGraph: {
      title: dict.modder?.seo?.title || "Auto vast in de modder? | Tips & Hulp",
      description: dict.modder?.seo?.description || "Is je auto vast in de modder? Lees wat je moet doen en krijg direct hulp. 24/7 bereikbaar voor pechhulp en berging.",
      type: "website",
      images: [
        {
          url: "/services/1.jpg",
          width: 1200,
          height: 630,
          alt: dict.modder?.seo?.imageAlt || "Auto vast in de modder",
        },
      ],
    },
    twitter: {
      card: "summary_large_image",
      title: dict.modder?.seo?.title || "Auto vast in de modder? | Tips & Hulp",
      description: dict.modder?.seo?.description || "Is je auto vast in de modder? Lees wat je moet doen en krijg direct hulp. 24/7 bereikbaar voor pechhulp en berging.",
      images: ["/services/1.jpg"],
    },
    alternates: {
      canonical: `/${locale}/wat-te-doen-als-je-auto-vast-zit-in-de-modder`,
    },
  }
}

export default async function ModderPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  const { locale } = await params
  const dict = await getDictionary(locale as "nl" | "en" | "fr" | "de")

  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      {/* Hero Section */}
      <section
        className="relative text-white flex items-center"
        style={{
          backgroundImage: "url(/services/1.jpg)",
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        <div className="absolute inset-0 bg-black/70"></div>
        <div className="relative w-full flex items-center py-16 md:py-32">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div className="max-w-3xl">
              <div className="space-y-8">
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-2 bg-[#c8eb67] text-black px-4 py-2 rounded-full text-sm font-medium">
                    <Car className="w-4 h-4" />
                    {dict.modder?.hero?.badge || "Auto vast in de modder"}
                  </div>
                  <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold leading-tight">
                    <span className="text-white">{dict.modder?.hero?.title?.part1 || "Auto vast in de modder?"}</span>
                    <br />
                    <span className="text-[#c8eb67]">{dict.modder?.hero?.title?.part2 || "Zo kom je weer los"}</span>
                  </h1>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <a
                    href={`tel:${dict.modder?.hero?.phone || "0852502928"}`}
                    className="w-full sm:w-auto inline-flex items-center justify-between px-6 py-3 bg-[#c8eb67] text-black rounded-full shadow-lg hover:bg-[#b8db57] transition-all group text-base font-semibold"
                  >
                    <span className="flex items-center gap-3">
                      <Phone className="w-5 h-5" />
                      {dict.modder?.hero?.cta || "Bel Direct: 085-250 29 28"}
                    </span>
                  </a>
                  <Link
                    href="https://wa.me/31850609880"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full sm:w-auto inline-flex items-center justify-between rounded-full bg-white text-black border border-gray-300 hover:shadow-lg px-6 py-3 font-semibold transition-all group text-base"
                  >
                    <span className="mr-3">{dict.modder?.hero?.whatsapp || "WhatsApp"}</span>
                    <div className="w-8 h-8 rounded-full bg-[#c8eb67] text-black flex items-center justify-center transition-transform group-hover:translate-x-1">
                      <ArrowRight className="w-4 h-4" />
                    </div>
                  </Link>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Tips Section */}
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              {dict.modder?.tips?.title || "Wat te doen als je auto vastzit in de modder?"}
            </h2>
            <p className="text-base sm:text-xl text-gray-600 max-w-3xl mx-auto">
              {dict.modder?.tips?.subtitle ||
                "Volg deze stappen om je auto veilig los te krijgen. Lukt het niet? Wij helpen je direct!"}
            </p>
          </div>
          <div className="space-y-8 max-w-2xl mx-auto">
            {(dict.modder?.tips?.items || [
              { title: "Blijf rustig", description: "Voorkom doorslippen door niet te veel gas te geven." },
              { title: "Maak ruimte vrij", description: "Verwijder zoveel mogelijk modder rondom de banden." },
              { title: "Gebruik matten of takken", description: "Leg iets onder de wielen voor extra grip." },
              { title: "Probeer voorzichtig te rijden", description: "Schakel naar een lage versnelling en probeer langzaam vooruit of achteruit te rijden." },
              { title: "Lukt het niet?", description: "Bel onze hulpdienst, wij zijn snel ter plaatse!" }
            ]).map((tip, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl shadow-lg flex items-start gap-4">
                <div className="w-10 h-10 bg-[#c8eb67] rounded-full flex items-center justify-center mt-1 flex-shrink-0">
                  <ArrowRight className="w-5 h-5 text-black" />
                </div>
                <div>
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">{tip.title}</h3>
                  <p className="text-gray-600">{tip.description}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Location Sharing */}
      <section className="py-16 bg-white">
        <div className="container mx-auto px-6 flex flex-col-reverse md:flex-row items-center gap-12">
          <div className="md:w-3/5 text-center md:text-left">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              {dict.modder?.locatiesectie?.title || "Deel uw locatie voor hulp"}
            </h2>
            <p className="text-gray-700 mb-8 text-base sm:text-lg leading-relaxed">
              {dict.modder?.locatiesectie?.description ||
                "Deel uw locatie zodat wij u snel kunnen helpen en uw auto veilig kunnen bergen."}
            </p>
            <LocationButton
              dict={dict.modder?.locatiebutton || { sendLocation: "Deel Locatie", loading: "Laden..." }}
            />
          </div>
          <div className="md:w-2/5 flex justify-center mb-8 md:mb-0">
            <Image
              src="/location.gif"
              alt={dict.modder?.locatiesectie?.imageAlt || "Locatie delen voor hulp"}
              width={200}
              height={200}
              className="w-40 md:w-48 h-auto rounded-2xl"
            />
          </div>
        </div>
      </section>

      {/* Emergency CTA */}
      <section className="py-20 bg-[#c8eb67] text-black relative overflow-hidden">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mb-12">
            <AlertTriangle className="h-24 w-24 mx-auto mb-8 text-black" />
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-6">
              {dict.modder?.emergency?.title || "Direct hulp nodig? Bel nu!"}
            </h2>
            <p className="text-lg sm:text-2xl text-black mb-12 max-w-3xl mx-auto">
              {dict.modder?.emergency?.subtitle ||
                "24/7 bereikbaar voor hulp bij vastzittende auto's. Snel ter plaatse, altijd veilig geholpen."}
            </p>
          </div>
          <div className="bg-black rounded-3xl p-8 md:p-12 mb-16 max-w-lg mx-auto">
            <Phone className="h-12 md:h-16 w-12 md:w-16 mx-auto mb-6 text-white" />
            <a href={`tel:0852502928`}>
              <div className="text-3xl sm:text-5xl md:text-6xl text-white font-bold mb-4">
                {dict.modder?.emergency?.phone || "085-250 29 28"}
              </div>
            </a>
            <div className="text-white text-lg sm:text-xl">
              {dict.modder?.emergency?.phoneSubtitle || "Direct verbonden met onze hulpdienst"}
            </div>
          </div>
        </div>
      </section>

      {/* FAQ Section */}
      <FAQSection dict={dict} />
    </div>
    )
}