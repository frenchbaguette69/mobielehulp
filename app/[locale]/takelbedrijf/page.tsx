...
export async function generateMetadata({
  ...
  return {
    title: dict.takelbedrijf?.seo?.title || "Takelbedrijf - 24/7 Snel en Veilig Takeldienst voor Auto's | Mobiele Hulp",
    description: dict.takelbedrijf?.seo?.description || "Direct een takelbedrijf nodig? 24/7 bereikbaar, snelle service, professioneel en veilig uw auto laten takelen of bergen.",
    keywords: dict.takelbedrijf?.seo?.keywords || "takelbedrijf, takeldienst, auto takelen, auto berging, 24/7 takeldienst, pechhulp, auto transport",
    openGraph: {
      title: dict.takelbedrijf?.seo?.title || "Takelbedrijf - 24/7 Snel en Veilig Takeldienst voor Auto's",
      description: dict.takelbedrijf?.seo?.description || "Direct een takelbedrijf nodig? 24/7 bereikbaar, snelle service, professioneel en veilig uw auto laten takelen of bergen.",
      type: "website",
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: dict.takelbedrijf?.seo?.imageAlt || "Professioneel takelbedrijf",
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `/${locale}/takelbedrijf`,
    },
  }
}

export default async function TakelbedrijfPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
              ...
              <div className="space-y-8">
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-2 bg-[#c8eb67] text-black px-4 py-2 rounded-full text-sm font-medium">
                    ...
                    {dict.takelbedrijf?.hero?.badge || "Takelbedrijf"}
                  </div>
                  <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold leading-tight">
                    <span className="text-white">{dict.takelbedrijf?.hero?.title?.part1 || "Takelbedrijf Nodig?"}</span>
                    <br />
                    <span className="text-[#c8eb67]">{dict.takelbedrijf?.hero?.title?.part2 || "Snel & Veilig Getakeld"}</span>
                  </h1>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <a
                    href={`tel:${dict.takelbedrijf?.hero?.phone || "0852502928"}`}
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      {dict.takelbedrijf?.hero?.cta || "Bel Direct: 085-250 29 28"}
                    </span>
                  </a>
                  <Link
                  ...
                  >
                    <span className="mr-3">{dict.takelbedrijf?.hero?.whatsapp || "WhatsApp"}</span>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              {dict.takelbedrijf?.features?.title || "Waarom Ons Takelbedrijf?"}
            </h2>
            <p className="text-base sm:text-xl text-gray-600 max-w-3xl mx-auto">
              {dict.takelbedrijf?.features?.subtitle ||
                "Ons takelbedrijf is 24/7 bereikbaar en zorgt voor veilig, snel en professioneel takelen of bergen van uw auto."}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {dict.takelbedrijf?.features?.items?.map((feature, index) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-8">
                {dict.takelbedrijf?.serviceDetails?.title || "Complete Takelbedrijf Service"}
              </h2>
              <div className="space-y-6">
                {dict.takelbedrijf?.serviceDetails?.items?.map((service, index) => (
            ...
            <div className="relative mt-10 lg:mt-0">
              <Image
                src="/services/4.jpg"
                alt={dict.takelbedrijf?.serviceDetails?.imageAlt || "Professioneel takelbedrijf"}
                width={600}
                height={400}
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              {dict.takelbedrijf?.situations?.title || "Wanneer Takelbedrijf Nodig?"}
            </h2>
            <p className="text-base sm:text-xl text-gray-600 max-w-3xl mx-auto">
              {dict.takelbedrijf?.situations?.description ||
                "Of het nu gaat om pech, een ongeval of transport, ons takelbedrijf helpt u snel en veilig."}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {dict.takelbedrijf?.situations?.items?.map((situation, index) => (
              <div
                key={index}
        ...
        <div className="container mx-auto px-6 flex flex-col-reverse md:flex-row items-center gap-12">
          <div className="md:w-3/5 text-center md:text-left">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              {dict.takelbedrijf?.locatiesectie?.title || "Deel Uw Locatie voor Takelbedrijf"}
            </h2>
            <p className="text-gray-700 mb-8 text-base sm:text-lg leading-relaxed">
              {dict.takelbedrijf?.locatiesectie?.description ||
                "Geef uw locatie door zodat wij u snel kunnen helpen en uw auto veilig kunnen takelen of bergen."}
            </p>
            <LocationButton
              dict={dict.takelbedrijf?.locatiebutton || { sendLocation: "Deel Locatie", loading: "Laden..." }}
            />
          </div>
          <div className="md:w-2/5 flex justify-center mb-8 md:mb-0">
            <Image
              src="/location.gif"
              alt={dict.takelbedrijf?.locatiesectie?.imageAlt || "Locatie delen voor takelbedrijf"}
              width={200}
              height={200}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mb-12">
            ...
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-6">
              {dict.takelbedrijf?.emergency?.title || "Direct Takelbedrijf Nodig? Bel Nu!"}
            </h2>
            <p className="text-lg sm:text-2xl text-black mb-12 max-w-3xl mx-auto">
              {dict.takelbedrijf?.emergency?.subtitle ||
                "24/7 bereikbaar voor takelbedrijf. Snel ter plaatse, altijd veilig vervoer of berging."}
            </p>
          </div>
          <div className="bg-black rounded-3xl p-8 md:p-12 mb-16 max-w-lg mx-auto">
            ...
            <a href={`tel:0852502928`}>
              <div className="text-3xl sm:text-5xl md:text-6xl text-white font-bold mb-4">
                {dict.takelbedrijf?.emergency?.phone || "085-250 29 28"}
              </div>
            </a>
            <div className="text-white text-lg sm:text-xl">
              {dict.takelbedrijf?.emergency?.phoneSubtitle || "Direct verbonden met onze takelbedrijf specialisten"}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {dict.takelbedrijf?.emergency?.stats?.map((stat, index) => (
